<div align="center">
  ...
  [![Version](https://img.shields.io/badge/版本-v1.4.53-32C98A?style=flat-square)](#版本与更新)
...
当前发布版本为 **v1.4.53（2026-08-12）**。完整逐项记录见 [CHANGELOG.md](./CHANGELOG.md)。

| 版本 | 面向使用者的主要更新 |
| --- | --- |
| **v1.4.55** | ??????????????????????????????? 0 ???
| **v1.4.54** | ?????????????????????????????????????????? 修复好友任务处理结束后仍停在好友农场、后续巡检持续空转的问题，增加有界返回自家确认 |
