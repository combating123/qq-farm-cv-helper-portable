<div align="center">
  ...
  [![Tests](https://img.shields.io/badge/发布回归-725%20passed-17345F?style=flat-square)](#版本与更新)
...
| 能力 | 做什么 | 使用价值 |
| --- | --- | --- |
| v1.4.40 当前运行纠正版 | 以用户恢复后正在运行的 v455 文件为唯一代码基线；保留免费福利封顶、人工领取证明、精确联系人分享、奖励分离与会员状态说明；发布包移除用户数据、日志、备份和现场截图 | 运行文件哈希锁定；736 项发现（725 通过、11 项旧契约跳过）；现场响应验证 |
...
当前发布版本为 **v1.4.40（2026-08-10）**。该版本精确采用用户恢复后正在运行的 v455 代码基线；`v1.4.39` 已撤回且不提供 Release。完整逐项记录见 [CHANGELOG.md](./CHANGELOG.md)。

| 版本 | 重点变化 | 验证 |
| --- | --- | --- |
| **v1.4.40** | 恢复并锁定当前正在运行的 v455 基线；保留免费福利同日封顶、人工领取状态、精确联系人分享与奖励分离、会员状态说明和永久免费声明 | 运行文件 SHA-256 锁定；固定 24 格账本 50/50；736 项发现（725 通过、11 项旧契约跳过）；当前进程响应验证 |`r`n| ~~v1.4.39~~ | 已撤回的错误发布候选，不作为下载版本 | 未创建 Release |
